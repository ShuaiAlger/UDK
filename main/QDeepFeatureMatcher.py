...
from torchvision.models.quantization import resnet18, resnet50, resnext101_32x8d, resnext101_64x4d
from torchvision.models.quantization import shufflenet_v2_x0_5, shufflenet_v2_x1_0, shufflenet_v2_x1_5, shufflenet_v2_x2_0
from torchvision.models.quantization import googlenet, inception_v3, mobilenet_v3_large
...
qmodel_list = [ "qresnet18", "qresnet50",
                "qresnext101_32x8d", "qresnext101_64x4d",
                "qshufflenet_v2_x0_5", "qshufflenet_v2_x1_0", "qshufflenet_v2_x1_5", "qshufflenet_v2_x2_0",
                "qgooglenet", "qinception_v3", "qmobilenet_v3_large"]


class QDeepFeatureMatcher(torch.nn.Module):
    def __init__(self, model: str = 'qresnet50', device = None, bidirectional=True, enable_two_stage = True, ratio_th = [0.9, 0.9, 0.9, 0.9, 0.95, 1.0]):
        super(QDeepFeatureMatcher, self).__init__()
        ...
        # model = "qshufflenet_v2_x2_0" # 22.35 37.89 54.53 67.28

        # model = "qmobilenet_v3_large" # 22.26 36.56 52.65 61.25

        # model = "qgooglenet" # 27.78 43.50 60.10 76.10

        # model = "qresnext101_64x4d" # 31.80 49.25 65.15 74.55
        ...
        if model == 'qresnet18':
            ...
            self.qmodel = resnet50(pretrained=True, quantize=True).to(self.device)
            self.model = create_feature_extractor(self.qmodel, return_nodes=self.return_nodes).to(self.device)
            self.upsample = torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.padding_n = 16
            print('model is loaded.')

        elif model == 'qresnext101_32x8d':
            self.return_nodes = {
                'relu': 'relu',
                'layer1': 'layer1',
                'layer2': 'layer2',
            ...
            self.qmodel = resnext101_32x8d(pretrained=True, quantize=True).to(self.device)
            self.model = create_feature_extractor(self.qmodel, return_nodes=self.return_nodes).to(self.device)
            self.upsample = torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.padding_n = 16
            print('model is loaded.')
        elif model == 'qresnext101_64x4d':
            self.return_nodes = {
                'relu': 'relu',
                'layer1': 'layer1',
                'layer2': 'layer2',
            ...
            self.qmodel = resnext101_64x4d(pretrained=True, quantize=True).to(self.device)
            print(self.qmodel)
            self.model = create_feature_extractor(self.qmodel, return_nodes=self.return_nodes).to(self.device)
            self.upsample = torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.padding_n = 16
        ...
        elif model == 'qshufflenet_v2_x0_5':
            self.return_nodes = {
                'conv1': 'conv1',
                'maxpool': 'maxpool',
                'stage2.0': 'stage2.0',
            ...
            self.qmodel = shufflenet_v2_x0_5(pretrained=True, quantize=True).to(self.device)
            print(self.qmodel)
            self.model = create_feature_extractor(self.qmodel, return_nodes=self.return_nodes).to(self.device)
            self.upsample = torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.padding_n = 16
            print('model is loaded.')
        elif model == 'qshufflenet_v2_x1_0':
            self.return_nodes = {
                'conv1': 'conv1',
                'maxpool': 'maxpool',
                'stage2.0': 'stage2.0',
            ...
            self.qmodel = shufflenet_v2_x1_0(pretrained=True, quantize=True).to(self.device)
            print(self.qmodel)
            self.model = create_feature_extractor(self.qmodel, return_nodes=self.return_nodes).to(self.device)
            self.upsample = torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.padding_n = 16
            print('model is loaded.')
        elif model == 'qshufflenet_v2_x1_5':
            self.return_nodes = {
                'conv1': 'conv1',
                'maxpool': 'maxpool',
                'stage2.0': 'stage2.0',
            ...
            self.qmodel = shufflenet_v2_x1_5(pretrained=True, quantize=True).to(self.device)
            print(self.qmodel)
            self.model = create_feature_extractor(self.qmodel, return_nodes=self.return_nodes).to(self.device)
            self.upsample = torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.padding_n = 16
            print('model is loaded.')
        elif model == 'qshufflenet_v2_x2_0':
            self.return_nodes = {
                'conv1': 'conv1',
                'maxpool': 'maxpool',
                'stage2.0': 'stage2.0',
            ...
            self.qmodel = shufflenet_v2_x2_0(pretrained=True, quantize=True).to(self.device)
            print(self.qmodel)
            self.model = create_feature_extractor(self.qmodel, return_nodes=self.return_nodes).to(self.device)
            self.upsample = torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.padding_n = 16
            ...
            self.qmodel = googlenet(pretrained=True, quantize=True).to(self.device)
            print(self.qmodel)
            self.model = create_feature_extractor(self.qmodel, return_nodes=self.return_nodes).to(self.device)
            self.upsample = torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.padding_n = 16
            print('model is loaded.')
        elif model == 'qinception_v3':
            self.return_nodes = {
                'Conv2d_1a_3x3': 'Conv2d_1a_3x3',
                'maxpool1': 'maxpool1',
                'maxpool2': 'maxpool2',
                'Mixed_6a.branch3x3': 'Mixed_6a.branch3x3',
                'Mixed_6a.branch3x3dbl_3': 'Mixed_6a.branch3x3dbl_3',
            }
            self.qmodel = inception_v3(pretrained=True, quantize=True).to(self.device)
            print(self.qmodel)
            self.model = create_feature_extractor(self.qmodel, return_nodes=self.return_nodes).to(self.device)
            self.upsample = torch.nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.padding_n = 16
        ...
        elif model == 'qmobilenet_v3_large':
            self.return_nodes = {
