...
class DeepFeatureMatcher(torch.nn.Module):
    ...
    def match(self, img_A, img_B, display_results=0, *args):
        ...
        # get acitvations
        activations_A_temp = self.model.forward(inp_A)
        activations_B_temp = self.model.forward(inp_B)
        
        activations_A = []

        for i in range(len(activations_A_temp)):
            activations_A.append(activations_A_temp[i])

        activations_B = []

        for i in range(len(activations_B_temp)):
            activations_B.append(activations_B_temp[i])

        # for i in range(len(activations_A_temp)):
        #     half_c = activations_A_temp[i].shape[1]//2
        #     activations_A.append(activations_A_temp[i][:, :half_c, :, :])

        # activations_B = []

        # for i in range(len(activations_B_temp)):
        #     half_c = activations_B_temp[i].shape[1]//2
        #     activations_B.append(activations_B_temp[i][:, :half_c, :, :])
        ...
        descriptor_A = activations_A[-1][0][:, y, x].cpu().numpy()
        descriptor_A = (descriptor_A - descriptor_A.min()) / (descriptor_A.max()-descriptor_A.min())

        descriptor_C = activations_C[-1][0][:, y, x].cpu().numpy()
        descriptor_C = (descriptor_C - descriptor_C.min()) / (descriptor_C.max()-descriptor_C.min())



        cv2.imshow("hist_A", desc2hist(descriptor_A))
        cv2.imshow("hist_C", desc2hist(descriptor_C))

        cv2.imshow("hist_A-C", desc2hist(np.abs(descriptor_A - descriptor_C)))
        ...
        if self.enable_two_stage:
            ...
            if points_A.size(1) >= 4:
                ...
                # may be use pydegensac for the homography matrix estimation of two_stage method
        ...
        # px_ths = [5.0, 5.0, 9.0, 9.0, 15.0, 15.0]

        px_ths = [30.0, 30.0, 30.0, 20.0, 20.0, 20.0]

        confs = [0.999, 0.999, 0.999, 0.9999, 0.9999, 0.9999]

        max_iters = [1000, 10000, 10000, 10000, 10000, 10000]


        # initiate matches modify
        ...
        if USE_CASCADE_RANSAC:
            if (points_A.shape[-1] > 8) or (points_A.shape[-2] > 8):
                model, inliers = pydegensac.findFundamentalMatrix(points_A.numpy().T, points_C.numpy().T, px_ths[-1], confs[-1], max_iters[-1])
                points_A, points_C = points_A[:, inliers], points_C[:, inliers]

        # initiate matches
        points_A, points_C = dense_feature_matching(activations_A[-2], activations_C[-2], self.ratio_th[-2], self.bidirectional)
        if USE_CASCADE_RANSAC:
            if (points_A.shape[-1] > 8) or (points_A.shape[-2] > 8):
                model, inliers = pydegensac.findFundamentalMatrix(points_A.numpy().T, points_C.numpy().T, px_ths[-2], confs[-2], max_iters[-2])
                points_A, points_C = points_A[:, inliers], points_C[:, inliers]
        ...
        for k in range(len(activations_A) - 3, -1, -1):
            ...
            if USE_CASCADE_RANSAC:
                if (points_A.shape[-1] > 8) or (points_A.shape[-2] > 8):
                    model, inliers = pydegensac.findFundamentalMatrix(points_A.numpy().T, points_C.numpy().T, px_ths[k], confs[k], max_iters[k])
                    points_A, points_C = points_A[:, inliers], points_C[:, inliers]
