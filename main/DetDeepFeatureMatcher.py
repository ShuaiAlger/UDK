...
from torchvision.models.detection.faster_rcnn import fasterrcnn_resnet50_fpn, fasterrcnn_resnet50_fpn_v2, fasterrcnn_mobilenet_v3_large_fpn, fasterrcnn_mobilenet_v3_large_320_fpn
...
from torchvision.models.detection.mask_rcnn import maskrcnn_resnet50_fpn, maskrcnn_resnet50_fpn_v2
from torchvision.models.detection.retinanet import retinanet_resnet50_fpn, retinanet_resnet50_fpn_v2
from torchvision.models.detection.ssdlite import ssdlite320_mobilenet_v3_large

from torchvision.models.feature_extraction import create_feature_extractor


class DetDeepFeatureMatcher(torch.nn.Module):
    def __init__(self, model: str = 'ssd300_vgg16', device = None, bidirectional=True, enable_two_stage = True, ratio_th = [0.9, 0.9, 0.9, 0.9, 0.95, 1.0]):
        super(DetDeepFeatureMatcher, self).__init__()
        ...
        if model == 'ssd300_vgg16':
            self.return_nodes = { }
            self.return_nodes.update({'backbone.features.1':'backbone.features.1'})
            self.return_nodes.update({'backbone.features.4':'backbone.features.4'})
            self.return_nodes.update({'backbone.features.9':'backbone.features.9'})
            self.return_nodes.update({'backbone.features.16':'backbone.features.16'})

            # self.model = ssd300_vgg16(pretrained=True, weights=SSD300_VGG16_Weights.COCO_V1).to(self.device).eval()

            self.smodel = ssd300_vgg16(pretrained=True, weights=SSD300_VGG16_Weights.COCO_V1).to(self.device).eval()
            print(self.smodel)
